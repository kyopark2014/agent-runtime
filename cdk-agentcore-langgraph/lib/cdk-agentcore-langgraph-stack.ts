...
const projectName = `agentcore-langgraph`; 
const region = process.env.CDK_DEFAULT_REGION;    
const accountId = process.env.CDK_DEFAULT_ACCOUNT;
const bucketName = `storage-for-${projectName}-${accountId}-${region}`; 
const vectorIndexName = projectName

export class CdkAgentcoreLanggraphStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const ECRImageAccessPolicy = new iam.PolicyStatement({  
      resources: [`arn:aws:ecr:${region}:${accountId}:repository/${projectName}`],
